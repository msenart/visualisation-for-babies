...
where $|p|_{dpt}$ is the number of births to people with the name $p$ in the department $dpt$.

- **Map 2** — When clicking any department $R$ on Map 1, Map 2 is colored according to the $η_{k,S,R}$ metric (with $k=3$) to compare its naming culture to every other department $S$.
...
The lack of color uniformity in the maps at some decades for certain departments reveals a very clear regional effect in naming data. 
Map 1 emphasizes the departments that deviate from the national pattern (regarding the most popular name)—and thus answers the 3rd question—while Map 2 highlights the departments that deviate from the trend of the selected region (regarding the top-$k$ names); in both cases, the departments that differenciate themselves from the reference are colored in orange-red, a color that catches the eye right away. The tooltips that appear when hovering over departments reveal the top five names—and thus allow easily to answer the 2nd question—and give the exact number of births associated to each name as well as the proportion by gender.

**Strenghts**
- The visualisation makes it easy to switch from a view based on national reference to one based on the selected region, allowing users to identify similarities and differences between departments from different perspectives. 
...
**Weaknesses**
- The understanding of the visualisation may not be immediate as the metric is quite abstract and requires to read carefully the formulas of $\eta_{dpt}$ and $η_{k,S,R}$.
- The $\eta$ metric is highly sensitive to the department size; in sparsely populated departments, even a small number of data points is enough to cause significant variations in scores.
