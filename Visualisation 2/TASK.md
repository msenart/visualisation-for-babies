...
We will implement (if Altair allows it) a Visualisation constituted of two maps. **The first one** is a map of France, in which **the departments are coloured** according to a certain number that tells about how is the global name ranking different from the regional name ranking. For example, we could have :
...
$$
\eta_{k,region} = \frac{\sum_{i=1}^{k}|\text{i-st most famous name in the region}|_{region}}{\sum_{i=1}^{k}|\text{i-st most famous name in the country}|_{region}} \geq 1
$$

Where $|.|_{region}$ is like doing the SQL operation ``SELECT COUNT * FROM table WHERE region = "region"``.

The higher the $\eta$ is ($\eta=1$ meaning that the regional ranking is exactly the same as the global ranking), the more the region is coloured, meaning that the region doesn't match with the global ranking, that there is discrepancies between **the local top and the global top**.
...
$$\omega_{name} = \frac{|\text{number of people having the name in the region}|_{region}}{|\text{total number of people in the region}|_{region}}$$
...
$$
\eta_{k,\text{selected region},region} = \frac{\sum_{i=1}^{k}|\text{i-st most famous name in the region}|_{region}}{\sum_{i=1}^{k}|\text{i-st most famous name in the selected region}|_{region}} \geq 1
$$
